...
export default function JiraBoard() {
  ...
  return (
    <div style={{ height: "100%", display: "flex", flexDirection: "column", background: "#f4f5f7", fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif" }}>
      <div style={{ background: "#0052CC", padding: "8px 16px", display: "flex", alignItems: "center", gap: 12, flexShrink: 0 }}>
        ...
        {cols.map(col => (
          <div key={col.key} style={{ background: "#ebecf0", borderRadius: 6, padding: 8, display: "flex", flexDirection: "column", overflow: "hidden" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 8, padding: "0 4px" }}>
              ...
              <span style={{ fontSize: 12, fontWeight: 600, color: "#5e6c84", textTransform: "uppercase", letterSpacing: 0.5 }}>{col.label}</span>
              <span style={{ fontSize: 11, color: "#8993a4", marginLeft: "auto" }}>{col.items.length}</span>
            </div>
            <div style={{ flex: 1, overflow: "auto", display: "flex", flexDirection: "column", gap: 6 }}>
              {col.items.map(item => (
                <div key={item.id} onClick={() => setSelected(selected?.id === item.id ? null : item)} style={{ background: "white", borderRadius: 4, padding: "10px 10px 8px", cursor: "pointer", boxShadow: "0 1px 2px rgba(0,0,0,0.1)", borderLeft: `3px solid ${col.color}` }}>
                  <div style={{ fontSize: 13, fontWeight: 500, color: "#172b4d", marginBottom: 4 }}>{item.name}</div>
                  {selected?.id === item.id && <div style={{ fontSize: 11, color: "#5e6c84", marginBottom: 6, lineHeight: 1.5 }}>{item.desc}</div>}
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    <span style={{ fontSize: 10, color: "#8993a4" }}>{item.id}</span>
                    <span style={{ fontSize: 10, color: "#8993a4", background: "#f4f5f7", padding: "1px 6px", borderRadius: 3 }}>{item.tech?.split("·")[0]?.trim()}</span>
                  </div>
                  {item.priority && <div style={{ marginTop: 4 }}><span style={{ fontSize: 9, fontWeight: 600, textTransform: "uppercase", color: item.priority === "high" ? "#de350b" : "#ff8b00", background: item.priority === "high" ? "#ffebe6" : "#fff3e0", padding: "1px 5px", borderRadius: 2 }}>{item.priority}</span></div>}
                </div>
              ))}
            </div>
          </div>
